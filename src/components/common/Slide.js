...
const Slide = forwardRef(({
  ...
  const toggle = (action, direction) => {
    return new Promise(resolve => {
      ...
      // Clear any running animations first to prevent flicker
      gsap.killTweensOf([imgReveal.current, imgWrap.current, heading.current, title.current]);

      // Ensure initial state before animation
      if (action === 'show') {
        // Set initial states immediately to prevent flicker
        gsap.set(imgWrap.current, {
          opacity: imgOpts.startAt?.opacity || 0,
          x: imgOpts.startAt?.x || 0,
          y: imgOpts.startAt?.y || 0
        });
        gsap.set(heading.current, {
          opacity: titleOpts.startAt?.opacity || 0,
          x: titleOpts.startAt?.x || 0,
          y: titleOpts.startAt?.y || 0,
          scale: titleOpts.startAt?.scale || 1
        });
  ...
  const togglePreview = (action) => {
    return new Promise(resolve => {
      ...
      gsap.killTweensOf([
        preview.imgReveal.current, 
        preview.imgWrap.current, 
        preview.close.current, 
        preview.heading.current, 
        preview.title.current, 
        preview.content.current
      ]);

      // Set initial states if showing
      if (action === 'show') {
        gsap.set(preview.imgWrap.current, { 
          opacity: 0, 
          y: '20%' 
        });
        gsap.set([preview.close.current, preview.heading.current, preview.title.current, preview.content.current], { 
          opacity: 0, 
          y: '200%' 
        });
      }
    ...
    <div className={`slide ${isCurrent ? styles.slideCurrent : ''}`} ref={el}>
      ...
      <div className="preview" ref={preview.container}>
        <div className="preview__img-wrap" ref={preview.imgWrap}>
          <div className="preview__img" ref={preview.img}></div>
        </div>
